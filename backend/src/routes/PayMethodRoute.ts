...
payMethodRoute.route('/active')
    .get( cors(), async (req: Request, res: Response) => {
        try{
            ...
            let max: any = req?.query.page || 5;
            LogInfo(`Query param: ${id}`);
            const result: any = await controller.getActivePayMethods(page, max, id);
            res.json(result);
        } catch (error){
            res.json(error);
        }
    });
payMethodRoute.route('/deleted')
    .get( cors(), async (req: Request, res: Response) => {
        try{
            let page: any = req?.query.page || 1;
            let max: any = req?.query.page || 5;
            const result: any = await controller.getDeletedPayMethods(page, max);
            res.json(result);
        } catch (error){
            res.json(error);
        }
